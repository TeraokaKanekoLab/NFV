...
extern unsigned int nf_nat_ipv4_fn(void *priv, struct sk_buff *skb, const struct nf_hook_state *state, struct nf_nat_ipv4_multi_range_compat *mr);
...
unsigned int nf_nat_func(struct sk_buff *skb, const struct nf_hook_state *state)
{
  ...
  ret = nf_nat_ipv4_fn(NULL, skb, state, mr);
  return ret;
}
